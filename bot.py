...
#Berjalan ketika bot sudah siap digunakan. Bot akan menampilkan pesan di konsol / terminal ketika sudah siap.
@bot.event
async def on_ready():
    print(f'Bot is ready. Logged in as {bot.user}')

@bot.command(name='start') #Bot akan memperkenalkan dirinya dan menampilkan informasi tentang perintah yang tersedia.
async def start_command(ctx):
    await ctx.send("Halo! Saya adalah bot manajer proyek\nSaya akan membantu kamu menyimpan proyek dan informasi tentangnya!)")
    await info(ctx)

@bot.command(name='info') #Bot akan menampilkan informasi tentang perintah yang tersedia.
async def info(ctx):
    await ctx.send("""
...
@bot.command(name='new_project') #Bot akan membantu pengguna dalam membuat proyek baru dengan memberikan beberapa pertanyaan (nama,link,status) dan jawaban pengguna akan disimpan di dalam database.
...
@bot.command(name='projects')#Bot akan menampilkan semua proyek yang telah disimpan oleh pengguna.
...
@bot.command(name='skills') #Bot akan membantu pengguna menambahkan keterampilan yang diperlukan ke proyek yang telah disimpan.
...
@bot.command(name='delete') #Bot akan membantu pengguna untuk menghapus proyek yang telah disimpan.
...
@bot.command(name='update_projects') #Bot akan membantu pengguna untuk memperbarui informasiproyek yang telah disimpan.
